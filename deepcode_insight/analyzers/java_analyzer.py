"""
Java-specific code analyzer (Fixed version)

Analyzer chuyên biệt cho Java code với Tree-sitter matches API đúng
"""
...
class JavaAnalyzer(BaseCodeAnalyzer):
    ...
    def _check_empty_catch_blocks(self, root_node: Node, code: str, result: AnalysisResult):
        """Check for empty catch blocks"""
        try:
            catch_query = self.query_manager.get_query(self.language, 'empty_catch')
            matches = catch_query.matches(root_node)
            
            for pattern_index, captures in matches:
                if 'catch_clause' in captures:
                    for node in captures['catch_clause']:
    ...
    def _check_naming_conventions(self, root_node: Node, code: str, result: AnalysisResult):
        ...
        for pattern_index, captures in method_matches:
            if 'method_name' in captures:
                for node in captures['method_name']:
                    method_name = self._get_node_text(node, code)
        ...
        variable_matches = variables_query.matches(root_node)
        
        for pattern_index, captures in variable_matches:
            if 'local_var' in captures:
                for node in captures['local_var']:
