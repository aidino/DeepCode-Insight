"""
Python-specific code analyzer (Fixed version)

Analyzer chuyên biệt cho Python code với Tree-sitter matches API đúng
"""
...
class PythonAnalyzer(BaseCodeAnalyzer):
    ...
    def _check_missing_docstrings(self, root_node: Node, code: str, result: AnalysisResult):
        ...
        class_matches = classes_query.matches(root_node)
    ...
    def _check_bare_except(self, root_node: Node, code: str, result: AnalysisResult):
        ...
        try:
            ...
            matches = bare_except_query.matches(root_node)
            
            for pattern_index, captures in matches:
                if 'bare_except' in captures:
                    for node in captures['bare_except']:
                        result.add_issue(
                            'bare_except',
    ...
    def _check_unused_imports(self, root_node: Node, code: str, result: AnalysisResult):
        ...
        import_matches = imports_query.matches(root_node)
        
        imported_names = set()
        
        for pattern_index, captures in import_matches:
    ...
    def _check_lambda_usage(self, root_node: Node, code: str, result: AnalysisResult):
        ...
        try:
            ...
            lambda_matches = lambda_query.matches(root_node)
            
            for pattern_index, captures in lambda_matches:
                if 'lambda_expr' in captures:
                    for node in captures['lambda_expr']:
    ...
    def _check_comprehension_complexity(self, root_node: Node, code: str, result: AnalysisResult):
        ...
        try:
            ...
            comp_matches = comp_query.matches(root_node)
            
            for pattern_index, captures in comp_matches:
                for capture_name, nodes in captures.items():
                    for node in nodes:
